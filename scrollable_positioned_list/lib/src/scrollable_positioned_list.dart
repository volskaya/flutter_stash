...
class ItemScrollController {
  ...
  Future<void> scrollTo(
      {@required int index, double alignment = 0, @required Duration duration, Curve curve = Curves.linear}) {
...
class _ScrollablePositionedListState extends State<ScrollablePositionedList> with TickerProviderStateMixin {
  ...
  Future<void> _scrollTo(
      {@required int index, double alignment, @required Duration duration, Curve curve = Curves.linear}) async {
    if (index > widget.itemCount - 1) {
      index = widget.itemCount - 1;
    }
