...
class _PositionedListState extends State<PositionedList> {
  ...
  void initState() {
    ...
    if (widget.itemPositionNotifier != null) {
  ...
  @override
  void dispose() {
    if (widget.itemPositionNotifier != null) scrollController.removeListener(_schedulePositionNotificationUpdate);
    super.dispose();
  }

  @override
  void didUpdateWidget(PositionedList oldWidget) {
    assert((oldWidget.itemPositionNotifier != null) == (widget.itemPositionNotifier != null));
    if (widget.itemPositionNotifier != null) _schedulePositionNotificationUpdate();
  ...
  Widget _buildSeparatedListElement(int index) {
    if (index.isEven) {
      ...
      return widget.separatorBuilder?.call(context, index ~/ 2) ?? const SizedBox.shrink();
    }
  }
  ...
  void _schedulePositionNotificationUpdate() {
    assert(widget.itemPositionNotifier != null);
    ...
    if (!updateScheduled) {
      ...
      SchedulerBinding.instance!.addPostFrameCallback((_) {
        ...
        for (var element in registeredElements.value!) {
          ...
          if (box == null || viewport == null || key == null) return;
