...
class _WaveState extends State<Wave> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Both animations notify the same ChangeNotifier,
    // which reclips _WaveClipper
  ...
  bool _isWaveValueAtBounds(double value) {
    assert(value <= _waveController.upperBound &&
        value >= _waveController.lowerBound);
    return value == _waveController.lowerBound ||
        value == _waveController.upperBound;
  }
  ...
  void didUpdateWidget(Wave oldWidget) {
    if (oldWidget.value != widget.value) {
      ...
      _maybeToggleWaveAnimation(
          !willValueAnimate ? _animation.value : widget.value);
      ...
      _valueController
          .forward(from: 0)
          .whenComplete(() => _maybeToggleWaveAnimation(widget.value));
    }
...
class _WaveClipper extends CustomClipper<Path> {
  ...
  List<Offset> _generateHorizontalWavePath(Size size, double curve) {
    ...
    final waveList = List<Offset>(size.height.toInt() + 4);
    for (int i = -2; i < waveList.length - 2; i++) {
      final waveHeight = size.width / 40;
      final dx = math.sin((waveValue * 360 - i) % 360 * (math.pi / 180)) *
              waveHeight *
              curve +
          (size.width * value);
  ...
  List<Offset> _generateVerticalWavePath(Size size, double curve) {
    ...
    for (int i = -2; i < waveList.length - 2; i++) {
      ...
      final dy = math.sin((waveValue * 360 - i) % 360 * (math.pi / 180)) *
              waveHeight *
              curve +
