...
class MultiPhotoCarouselState extends State<MultiPhotoCarousel> with InitialDependencies<MultiPhotoCarousel> {
    ...
    if (widget.precacheSurroundingPhotos) {
      ...
      if (_photos?.isNotEmpty == true) {
        ...
        for (final photo in photos) {
          ...
          final isCached = PaintingBinding.instance.imageCache.containsKey(imageProvider) ||
  ...
  Widget build(BuildContext context) => LayoutBuilder(
        builder: (context, constraints) {
          final itemCount = math.max(_photos?.length ?? 0, 1);
